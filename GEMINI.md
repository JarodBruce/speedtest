...
*   **非ASCII文字の検証:** 日本語のような非ASCII文字を含むファイルを編集する際、文字化けが発生する可能性があります。これを検知し修正するため、以下の手順を必ず守ってください。
    ...
    3.  万が一、文字化けが確認された場合は、直ちに正しい内容でファイルを上書きして修正します。この修正が完了するまで、他のタスクに進ないでください。（`write_file`や`replace`で問題が解決しない場合は、`sed`のようなコマンドを使って解決します）。
...
## プロジェクトの目的
クライアントはブラウザで動作するダウンロードとアップロードを測定するサーバを作ってください
rustでwebsocketを使いTCPマルチコネクションで回線の最大値を正確に測定できるようにして
